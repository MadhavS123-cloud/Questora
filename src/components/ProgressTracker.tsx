...
export default function ProgressTracker({ status, step, logs, onBackgroundToggle, onCancel }: ProgressTrackerProps) {
  ...
  return (
    <div className="w-full max-w-3xl mx-auto font-ui animate-fade-slide-up">
      ...
      {/* ── Outer card ────────────────────────────────────────────── */}
      <div className="bg-white border border-neutral-100 rounded-2xl overflow-hidden shadow-[0_8px_40px_-8px_rgba(0,0,0,0.1)]">

        {/* Header */}
        <div className="bg-gradient-to-r from-neutral-900 via-slate-900 to-[#0a0f1e] px-6 py-5 flex items-center justify-between relative overflow-hidden">
          ...
          <div className="flex items-center gap-2 relative">
            <button
              ...
              className="px-3.5 py-1.5 bg-white/5 hover:bg-white/10 border border-white/10 text-slate-200 font-semibold text-xs rounded-lg transition-all flex items-center gap-1.5"
            >
            ...
            <button
              onClick={onCancel}
              className="px-3.5 py-1.5 bg-white/5 hover:bg-red-500/20 border border-white/10 hover:border-red-500/30 text-slate-400 hover:text-red-400 font-semibold text-xs rounded-lg transition-all"
            >
              Cancel
            </button>
          </div>
        ...
        <div className="p-6 space-y-6">
          ...
          <div className="space-y-2">
            <div className="flex items-center justify-between text-xs font-bold text-neutral-700">
              ...
              <span className="tabular-nums text-brand-600">{Math.round(progressPercent)}%</span>
            </div>
            <div className="relative w-full h-2 bg-neutral-100 rounded-full overflow-hidden">
              <div
                className="absolute left-0 top-0 h-full bg-gradient-to-r from-brand-600 via-indigo-500 to-semantic-ai rounded-full transition-all duration-700 ease-out"
              ...
              return (
                <div
                  ...
                  className={clsx(
                    ...
                    isFinished && 'border-emerald-100 bg-emerald-50/50',
                    isActive && 'border-brand-300 bg-brand-50/60 shadow-[0_0_12px_rgba(99,102,241,0.12)] scale-[1.02]',
                    isPending && 'border-neutral-100 bg-neutral-50/40 opacity-50'
                  )}
                >
                  {/* Step indicator */}
                  ...
                  <div>
                    <p className={clsx(
                      ...
                      isFinished && 'text-emerald-700',
                      isActive && 'text-brand-700',
                      isPending && 'text-neutral-500'
                    )}>
                      {stage.label}
                    </p>
                    <p className="text-[9px] text-neutral-400 mt-0.5 leading-tight hidden sm:block">
                      {stage.desc}
                    </p>
                  </div>
                  {/* Active pulse line */}
                  {isActive && (
                    <div className="absolute bottom-0 left-0 right-0 h-[2px] bg-gradient-to-r from-brand-400 to-semantic-ai rounded-b-xl" />
                  )}
                </div>
              );
            })}
          ...
          <div className="space-y-2">
            ...
            <div
              className="w-full h-48 bg-[#080d17] rounded-xl border border-slate-800/80 p-4 overflow-y-auto space-y-1 dark-scrollbar font-mono text-xs"
              aria-live="polite"
              aria-label="AI generation logs"
            >
              {/* Terminal prompt line */}
              <div className="text-slate-600 text-[10px] mb-2 border-b border-slate-900 pb-1.5 flex items-center gap-2">
                <span className="text-emerald-500 font-bold">questora@ai</span>
                <span className="text-slate-600">:</span>
                <span className="text-brand-400">~/synthesis</span>
                <span className="text-slate-600">$</span>
                <span className="text-slate-500">generate --mode=pedagogical --compliance=grade-10</span>
              </div>
                ...
                return (
                  <div
                    ...
                    className={clsx(
                      ...
                      isWarn && !isPhase && 'text-amber-400',
                      !isPhase && !isSuccess && !isWarn && 'text-slate-400'
                    )}
                  >
                    <span className="text-slate-700 select-none shrink-0 mt-0.5">&gt;&gt;</span>
                    <span>{log}</span>
                  </div>
                );
              })}
